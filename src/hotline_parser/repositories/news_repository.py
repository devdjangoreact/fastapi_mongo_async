...
class NewsRepository:
    ...
    async def get_news_by_source_and_date(
        self, source: str, until_date: datetime, limit: int = 100
    ) -> List[NewsItem]:
        """Get news from database by source and date"""
        try:
            cursor = (
                self.collection.find(
                    {
                        "source": source,
                        "article_data.published_at": {"$gte": until_date},
                    }
                )
            ...
            log.debug(
                f"Retrieved {len(news_items)} news items from database for {source}"
            )
            return news_items
